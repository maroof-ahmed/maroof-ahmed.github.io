(function ($) {
  ...
  $.fn.extend({
    animate: function animate(animationName) {
  ...
  $(document).ready(function () {
    ...
    // }, 0);
    document.querySelector(window.location.hash.length ? window.location.hash : '#home').scrollIntoView({
      behavior: 'smooth'
    });
    ...
    var wp2 = skillSection.waypoint({
      handler: function handler(direction) {
        if (direction == 'down') {
          $('.desc-wrap', skillSection).addClass('animated slideInLeft').removeClass('opacity-hide');
          $('.icon-wrap', skillSection).addClass('animated slideInRight').removeClass('opacity-hide');
        }
      },
      continuous: true,
    ...
    contactSection.waypoint({
      handler: function handler(direction) {
        if (direction == 'down') {
          $('.desc-wrap', contactSection).addClass('animated fadeInLeft').removeClass('hide');
          $('.icon-wrap', contactSection).addClass('animated fadeInRight').removeClass('hide');
        }
      },
      continuous: false,
      offset: '-40%'
